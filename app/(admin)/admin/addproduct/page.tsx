...
export default function FormElementsPage() {
  ...
  // ✅ Field arrays
  const { fields: addDescFields, append: addDescAppend, remove: addDescRemove } =
    useFieldArray({ control, name: "additionalDescriptions" });

  const { fields: thumbFields, append: thumbAppend, remove: thumbRemove } =
    useFieldArray({ control, name: "imgs.thumbnails" });

  const { fields: previewFields, append: previewAppend, remove: previewRemove } =
    useFieldArray({ control, name: "imgs.previews" });

  const { fields: optionFields, append: optionAppend, remove: optionRemove } =
  ...
  return (
    <div className="ml-4">
      {fields.map((field, i) => (
        <div key={field.id} className="flex gap-2 mb-2">
          <input
            {...register(`productOptions.${optionIndex}.optionValues.${i}.label`)}
            className="border p-2 rounded"
            placeholder="Label"
          />
          <input
            type="number"
            {...register(`productOptions.${optionIndex}.optionValues.${i}.value`)}
            className="border p-2 rounded"
            placeholder="Price"
          />
          <input
            type="number"
            {...register(`productOptions.${optionIndex}.optionValues.${i}.discount`)}
            className="border p-2 rounded"
