...
<div className="bg-gray-50 text-gray-900">
  <div className="min-h-screen p-6">
    ...
    <header className="flex flex-col md:flex-row md:items-center md:justify-between gap-4 mb-6">
      <div>
        <h1 className="text-2xl font-semibold">Order ID: {products?.orderNumber}</h1>
        <p className="text-sm text-gray-500">
            {}
            {moment(products?.createdAt).format("MMMM D, YYYY [at] h:mm a")} from Draft Orders
      ...
      <div className="flex flex-wrap gap-2">
        <span>
          ...
          {getStatusBadge(products?.paymentStatus)}
        </span>
      ...
      <div className="space-y-6">
          ...
          <div className="p-4 space-y-4">
            ...
            {products?.items?.map((pItems, index) => {
              ...
              <div className="flex-1">
                <p className="text-sm font-medium">{pItems?.description}</p>
              ...
              <div className="flex flex-col items-end gap-1">
                <div className="flex items-center border rounded-md overflow-hidden text-sm">
                  <span className="px-3 py-1 border-r bg-gray-50">{pItems?.quantity} x ${pItems?.amount_total}</span>
          ...
          <div className="p-4 space-y-4">
            ...
            <div className="space-y-2 text-sm">
              <div className="flex justify-between">
                ...
                <span className="text-gray-800">{products?.items?.length} item &nbsp; ${products?.totalAmount}</span>
              </div>
              <div className="flex justify-between">
                <span className="text-gray-600">Discount</span>
                <span className="text-green-700">New customer &nbsp; −$00</span>
              </div>
              <div className="flex justify-between">
                <span className="text-gray-600">Shipping</span>
                <span className="text-gray-800">Free shipping (0.0 lb) &nbsp; $0.00</span>
              </div>
        ...
        <section className="bg-white rounded-lg shadow-sm border">
          ...
          <div className="p-4 space-y-4">
            ...
            <select className="w-full border rounded-md text-sm p-2 focus:outline-none focus:ring-1 focus:ring-indigo-500">
                <option>-- Select Status --</option>
                <option>Order Under Proccess</option>
                <option>Order Shipped</option>
                <option>Order Out for Delivery</option>
                <option>Delivered</option>
            </select>
            <textarea
              className="w-full border rounded-md text-sm p-2 focus:outline-none focus:ring-1 focus:ring-indigo-500"
              rows={3}
              placeholder="Leave a comment..."
            ></textarea>
            <button className="border rounded-md text-sm p-2 bg-indigo-600 text-white focus:outline-none focus:ring-1 focus:ring-indigo-500">Submit</button>
          </div>
        </section>
      </div>
      ...
      <div className="space-y-6">
        ...
        <section className="bg-white rounded-lg shadow-sm border">
          ...
          <div className="px-4 pb-4 space-y-1 text-sm">
            <p className="font-medium">{products?.user?.first_name} {' '} {products?.user?.last_name}</p>
        ...
        <section className="bg-white rounded-lg shadow-sm border">
          ...
          <div className="px-4 pb-4 text-sm space-y-1">
            ...
            <p>{products?.user?.mobile ? products?.user?.mobile : 'No phone number'}</p>
        ...
        <section className="bg-white rounded-lg shadow-sm border">
          ...
          <div className="px-4 pb-4 text-sm space-y-1">
            <p className="font-medium">{products?.user?.userAddress[0]?.name}</p>
            <p>{products?.user?.userAddress[0]?.street}</p>
            <p>{products?.user?.userAddress[0]?.city}, {products?.user?.userAddress[0]?.state}, {products?.user?.userAddress[0]?.postalCode}</p>
