  ...
  if (loading) return <p>Loading...</p>;
    ...
    return(
        <div className="bg-gray-100 min-h-screen flex items-center justify-center p-4">
            <div  ref={targetRef} className="w-[800px]">
          <div className="bg-white rounded-2xl shadow-sm border overflow-hidden w-full max-w-[800px]">
              ...
              <div className="p-6">
                  <div className="flex flex-wrap justify-between items-center gap-4">
                      <div>
                          ...
                          <p className="text-slate-900 text-sm font-medium mt-2">#{invoice?.order.orderNumber}</p>
                      </div>
                      <div>
                          <p className="text-slate-500 text-sm font-medium">Date</p>
                          <p className="text-slate-900 text-sm font-medium mt-2">{moment(invoice?.createdAt).format("MMM DD, YYYY")}</p>
                      </div>
                      <div>
                          <p className="text-slate-500 text-sm font-medium">Total</p>
                          <p className="text-sm font-medium text-indigo-700 mt-2">${invoice?.order.totalAmount}</p>
                      </div>
                  </div>
                  ...
                  <div className="bg-gray-100 rounded-xl p-4 mt-8">
                      ...
                      <div className="grid sm:grid-cols-2 gap-4">
                          <div>
                              ...
                              <p className="text-slate-900 text-sm font-medium mt-2">{invoice?.invoice?.customerEmail}</p>
                          </div>
                          <div>
                              <p className="text-slate-500 text-sm font-medium">Payment Status</p>
                              <p className="text-slate-900 text-sm font-medium mt-2">{invoice?.order?.paymentStatus}</p>
                          </div>
                          {/* <div>
                  ...
                  <div className="mt-8">
                      ...
                      <div className="space-y-4">
                        {invoice?.order?.items?.map((product:InvoceItems, index:number) => (
                  ...
                  <div className="bg-gray-100 rounded-xl p-4 mt-8">
                      ...
                      <div className="space-y-4">
                          <div className="flex justify-between">
                              ...
                              <p className="text-slate-900 text-sm font-semibold">${invoice?.invoice?.amountSubtotal}</p>
                          </div>
                          <div className="flex justify-between">
                          ...
                          <div className="flex justify-between pt-3 border-t border-gray-300">
                              ...
                              <p className="text-[15px] font-semibold text-indigo-700">${invoice?.invoice?.amountSubtotal}</p>
                          </div>
                      </div>
                  </div>
              </div>
              ...
              <div className="bg-gray-100 px-6 py-4">
                  <div className="flex flex-col sm:flex-row justify-between items-center gap-4">
                      ...
                      <button onClick={() => toPDF()} className="bg-indigo-600 hover:bg-indigo-700 text-white font-medium text-[15px] py-2 px-4 rounded-lg max-sm:-order-1 cursor-pointer transition duration-200">
                          Download Invoice
                      </button>
                  </div>
              </div>
